            ...
            <div class="w-1/4 bg-white border-r max-h-screen">
                <div class="p-4 max-h-screen">
                    ...
                    <div class="space-y-4 overflow-y-auto h-[calc(100vh-160px)]">
                        ...
                        <div onclick="window.location='{{ url('/messages/'.$user->id) }}'" @class(['flex items-center space-x-4 p-3 hover:bg-gray-100 rounded-lg cursor-pointer', 'bg-gray-100'=> $user->id === $receiverUser->id])>
                            <img src="https://via.placeholder.com/40" alt="Alice Johnson" class="w-10 h-10 rounded-full">
                            <div class="flex justify-end">
                                <div>
                                    <p class="font-medium">{{ str($user->name)->headline() }}</p>
                                    <p class="text-sm text-gray-500">{{ str($user?->latestReceiverMessage?->message ?? 'Belum ada pesan')->limit(20) }}</p>
                                </div>
                                <!-- <div class="">
            ...
            <div class="flex-1 flex flex-col">
                ...
                <div class="flex-1 p-4 overflow-y-auto space-y-4 h-[calc(100vh)]">
                    ...
                    <div class="flex items-start justify-end">
                        <div class="bg-blue-500 text-white p-3 rounded-lg max-w-xs">
                            <p>{{ $message->message }}</p>
                            <p class="text-xs text-blue-100 mt-1">{{ $message->created_at->isoFormat('HH:mm') }}</p>
                        </div>
                    </div>
                    @else
                    <div class="flex items-start">
                ...
                <div class="bg-white p-4 border-t">
                    ...
                    <div class="flex items-center">
                        <input required type="text" name="message" placeholder="Type a message..." class="flex-1 px-4 py-2 border rounded-l-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-r-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            Send
                        </button>
                    </div>
                    </form>
