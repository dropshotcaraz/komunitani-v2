<x-app-layout>
    <div class="container mx-auto max-w-6xl my-12 px-4 py-8 bg-green-50">
        <div class="relative">
            <!-- Profile Header -->
            <div class="relative bg-gradient-to-r from-green-100 to-green-200 rounded-xl shadow-lg overflow-hidden">
                <!-- Cover Photo -->
                <div class="h-64 relative">
                    @if($user->cover_photo)
                        <img src="{{ asset("storage/{$user->cover_photo}") }}" class="w-full h-full object-cover">
                    @else
                        <div class="w-full h-full bg-green-300 opacity-50 flex items-center justify-center">
                    ...
                    <!-- Edit Profile Button -->
                    <a href="{{ route('profile.edit') }}" class="absolute top-4 right-4 bg-green-600 text-white px-4 py-2 rounded-full hover:bg-green-700 transition duration-300 flex items-center">
            ...
            <!-- Profile Tabs -->
            <div class="mt-8">
                <div class="flex border-b-2 border-green-300">
                    <button class="px-6 py-3 tab-button active text-green-700 font-semibold hover:bg-green-100 transition" data-tab="posts">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        Posts
                    </button>
                    <button class="px-6 py-3 tab-button text-green-700 font-semibold hover:bg-green-100 transition" data-tab="likes">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div id="posts-tab" class="tab-content bg-white rounded-b-xl shadow-md p-4">
                    @forelse($posts as $post)
                        <div class="bg-green-50 shadow-md p-6 rounded-xl mb-6 border border-green-200 transition hover:shadow-lg">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center">
                                    <img src="{{ $post->user->profile_picture ? asset("storage/{$post->user->profile_picture}") : asset('images/avatar1.png') }}" 
                                         alt="{{ $post->user->name }}" 
                                         class="w-12 h-12 rounded-full mr-4 border-2 border-green-300">
                                    <div>
                                        <h2 class="font-bold text-green-800">{{ $post->user->name }}</h2>
                                        <p class="text-green-600 text-sm">{{ $post->created_at->format('d M Y - H:i') }}</p>
                                    </div>
                                </div>
                                
                                @if(auth()->check() && (auth()->user()->id === $post->user_id || auth()->user()->is_admin))
                                    <div class="flex space-x-3">
                                        <a href="{{ route('posts.edit', $post->id) }}" class="text-green-500 hover:text-green-700 transition"> <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.379-8.379-2.828-2.828z" />
                                            </svg>
                                        </a>
                            ...
                            @if($post->image_path)
                                <img src="{{ asset('storage/public/'.$post->image_path) }}" alt="Post Image" class="w-full h-auto mt-4 rounded-lg border border-green-300 cursor-pointer" onclick="openImageModal('{{ asset("storage/{$post->image_path}") }}')">
                            @endif
                ...
                <!-- Likes Tab -->
                <div id="likes-tab" class="tab-content hidden bg-white rounded-b-xl shadow-md p-4">
                    @forelse($likedPosts as $likedPost)
                        <div class="bg-green-50 shadow-md rounded-lg p-4 mb-4 border border-green-200">
                            <h2 class="font-bold text-green-800">{{ $likedPost->title }}</h2>
                            <p class="text-gray-700">{{ $likedPost->content }}</p>
                        </div>
                    @empty
                        <p class="text-gray-500">No liked posts available.</p>
                    @endforelse
                </div>
            </div>
        </div>
