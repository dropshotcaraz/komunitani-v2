            ...
            <div id="postFeed">
                <div class="bg-[#F7F0CF] shadow-lg p-6 rounded-3xl border border-gray-200">
                    <div class="flex items-center justify-right mb-4">
                        <a
                            href="{{ auth()->user()->id === $post->user_id ? route('profile.show') : route('profile.view', $post->user->id) }}">
                            @if (isset($post->user->profile_picture) && $post->user->profile_picture)
                                <img src="{{ asset('storage/' . $post->user->profile_picture) }}"
                                    alt="{{ $post->user->name }}"
                                    class="w-[50px] h-12 rounded-full mr-4 border border-gray-300">
                            @else
                                <img src="{{ asset('https://xsgames.co/randomusers/avatar.php?g=pixel') }}"
                                    alt="{{ $post->user->name }}"
                                    class="w-[50px] h-12 rounded-full mr-4 border border-gray-300">
                            @endif
                        </a>
                        <div>
                            ...
                            <p class="text-gray-500 text-sm">
                                {{ $post->created_at->setTimezone('Asia/Jakarta')->format('d M Y - H:i') }}</p>
                            @if (!Auth::user()->isFollowing($post->user->id) && Auth::user()->id !== $post->user->id)
                                <form action="{{ route('follow', $post->user->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                                        class="mt-2 bg-[#6FA843] text-white px-4 py-1 rounded hover:bg-[#578432] transition">Follow</button>
                                </form>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <p class="font-bold text-xl text-[#6FA843]">{{ $post->title }}</p>
                            ...
                            <div class="flex space-x-4">
                                <a href="{{ route('posts.edit', $post->id) }}"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                        ...
                                        <path d="M21.2799 6.40005L11.7399 15.94C10.7899 16.89 7.96987 17.33 7.33987 16.7C6.70987 16.07 7.13987 13.25 8.08987 12.3L17.6399 2.75002C17.8754 2.49308 18.1605 2.28654 18.4781 2.14284C18.7956 1.99914 19.139 1.92124 19.4875 1.9139C19.8359 1.90657 20.1823 1.96991 20.5056 2.10012C20.8289 2.23033 21.1225 2.42473 21.3686 2.67153C21.6147 2.91833 21.8083 3.21243 21.9376 3.53609C22.0669 3.85976 22.1294 4.20626 22.1211 4.55471C22.1128 4.90316 22.0339 5.24635 21.8894 5.5635C21.7448 5.88065 21.5375 6.16524 21.2799 6.400 ```blade
05V6.40005Z" stroke="#0000FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                        <path
                                            d="M11 4H6C4.93913 4 3.92178 4.42142 3.17163 5.17157C2.42149 5.92172 2 6.93913 2 8V18C2 19.0609 2.42149 20.0783 3.17163 20.8284C3.92178 21.5786 4.93913 22 6 22H17C19.21 22 20 20.2 20 18V13"
                                            stroke="#0000FF" stroke-width="1.5" stroke-linecap="round"
                    ...
                    @if ($post->image_path)
                        <img src="{{ asset('storage/public/' . $post->image_path) }}" alt="Post Image"
                            class="w-[300px] sm:w-full h-auto mt-4 rounded-lg border border-[#434028] cursor-pointer"
                            onclick="openImageModal('{{ asset('storage/public/' . $post->image_path) }}')">
                    @endif

                    <!-- Image Modal -->
                    <div id="modalBackdrop"
                        class="fixed inset-0 bg-black bg-opacity-70 hidden transition-opacity duration-500"></div>
                    <div id="imageModal"
                        class="fixed inset-0 flex items-center justify-center hidden transition-transform duration-500 transform scale-75">
                        <div class="bg-white rounded-lg shadow-lg p-4">
                            <img id="modalImage" name="image" src="" alt="Modal Image" class="rounded-lg">
                            <button onclick="closeImageModal()"
                                class="mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-green-600 transition-colors">Close</button>
                        </div>
                    </div>

                    <div class="flex justify-between mt-4">
                        <button class="like-button flex items-center text-gray-500 transition"
                            data-post-id="{{ $post->id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="font-bold h-6 w-6 mr-2 heart-icon"
                        ...
                        <button class="flex items-center text-gray-500 hover:text-blue-500 transition comment-button"
                            data-post-id="{{ $post->id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none"
                                viewBox="0 0 24 24" ```blade <path stroke-linecap="round" stroke-linejoin="round"
                                stroke-width="2"
                                d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                            </svg> Comment (<span class="comment-count">{{ $post->comments()->count() }}</span>)
                        </button>

                        <button class="flex items-center text-gray-500 hover:text-yellow-500 transition"
                    ...
                    <div id="commentForm{{ $post->id }}" class="mt-4">
                        <form action="{{ route('posts.comment', $post->id) }}" method="POST" class="space-y-2"
                            onsubmit="hideCommentForm({{ $post->id }});">
                            @csrf
                            <textarea name="content" placeholder="Tulis komentar di sini"
                                class="w-full px-2 py-1 bg-gray-50 rounded-lg outline-none border border-gray-300 focus:border-[#6FA843]"></textarea>
                            <button type="submit"
                                class="bg-[#6FA843] text-white px-4 py-2 rounded-lg hover:bg-[#578432] transition">Kirim
                                Komentar</button>
                        </form>
                    </div>
                                ...
                                @if (auth()->check() && (auth()->user()->id === $comment->user_id || auth()->user()->name === 'Admin'))
                                    ...
                                    <form action="{{ route('comments.update', $comment->id) }}" method="POST"
                                        ...
                                        <input type="text" name="content" value="{{ $comment->content }}"
                                            class="w-full bg-gray-50 p-3 rounded-lg outline-none border border-gray-300 focus:border-[#6FA843]"
                                            required>
                                        <button type="submit"
                                            class="bg-blue-500 text-white px-4 py-1 rounded-lg hover:bg-blue-600 transition">Update</button>
                                    </form>
                                    <form action="{{ route('comments.destroy', $comment->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 text-white px-4 py-1 rounded-lg hover:bg-red-600 transition">Delete</button>
                                    </form>
                                @else
                ...
                fetch(`/api/follow/${userId}`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            alert(data.message);
                            location
                                .reload(); // Reload the page to update the follow button
                        } else {
