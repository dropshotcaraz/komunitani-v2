    ...
    <div class="bg-[#F7F0CF] shadow-lg p-6 rounded-3xl border border-gray-200">
        <div class="flex items-center justify-right mb-4">
            <a href="{{ auth()->user()->id === $post->user_id ? route('profile.show') : route('profile.view', $post->user->id) }}">
                ...
                @else
                    <img src="{{ asset('https://xsgames.co/randomusers/avatar.php?g=pixel') }}" alt="{{ $post->user->name }}" class="w-[50px] h-12 rounded-full mr-4 border border-gray-300">
                @endif
            </a>
            <div>
                <a href="{{ auth()->user()->id === $post->user_id ? route('profile.show') : route('profile.view', $post->user->id) }}">
...
05V6.40005Z" stroke="#0000FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <div class="flex justify-between mt-4">
            ...
            <button class="flex items-center text-gray-500 hover:text-blue-500 transition comment-button" data-post-id="{{ $post->id }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" ```blade
